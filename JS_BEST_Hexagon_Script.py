...
num_rot = 10000           # Number of rotation steps (0 to 30 degrees)
...
def diffraction_pattern(theta, chi=500):
    ...
    for i, th in enumerate(theta):
        ...
        if sin_th < 1e-10:
            ...
            # Integral of chi*cos(alpha) from 0 to pi/6 = chi*sin(pi/6) = chi/2
            integral = chi / 2
        else:
            # For large chi, the integral is dominated by stationary phase
            # near alpha = 0 and alpha = pi/6
            # Use Fresnel-type approximation
            
            # Contribution from alpha near 0
            x_max = chi * sin_th
            if x_max > 1:
                # Asymptotic: integral of sinc approaches sqrt(pi/(2*chi*sin_th))
                integral = np.sqrt(np.pi / (2 * chi * sin_th)) * chi
            else:
                # Small angle: use series expansion
...
ax.plot(theta_deg, P_total, 'b-', alpha=1)
