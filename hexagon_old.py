...
def fresnel_sp(d, n, n1, n2):
    ...
    Rs = np.clip(rs**2, 0.0, 1.0)
    Rp = np.clip(rp**2, 0.0, 1.0)
    Ts = 1.0 - Rs
    Tp = 1.0 - Rp
    return Rs, Rp, Ts, Tp
...
def trace_one_orientation(
    ...
    A ray is stored as a dict with keys:
        pos, direction, intensity_s, intensity_p, inside, interactions

    Returns:
        exits = list of (direction, intensity_s, intensity_p)
    """
    ...
    for y0 in y_samples:
        ray_pool.append({
            ...
            "intensity_s": initial_intensity,
            "intensity_p": initial_intensity,
            "inside": False,
            "interactions": 0,
        })
    ...
    while i < len(ray_pool):
        ...
        intensity_total = ray["intensity_s"] + ray["intensity_p"]
        if intensity_total < intensity_cutoff:
            continue
        ...
        Rs, Rp, Ts, Tp = fresnel_sp(ray["direction"], n_if, n1, n2)

        # Reflected branch
        I_ref_s = ray["intensity_s"] * Rs
        I_ref_p = ray["intensity_p"] * Rp
        if I_ref_s + I_ref_p > 0.0:
            ...
            if ray["inside"]:
                ...
                exits.append((d_ref, I_ref_s, I_ref_p))

        # Transmitted branch
        I_tr_s = ray["intensity_s"] * Ts
        I_tr_p = ray["intensity_p"] * Tp
        if I_tr_s + I_tr_p > 0.0:
            ...
            if d_tr is not None:
                ...
                if ray["inside"]:
                    # exiting crystal
                    exits.append((d_tr, I_tr_s, I_tr_p))
                else:
                    # entering crystal
                    ray_pool.append({
                        "pos": pos_tr,
...
def simulate_phase_function(
    ...
    hist_az_s = np.zeros(n_az_bins, dtype=float)
    hist_az_p = np.zeros(n_az_bins, dtype=float)
    hist_sc_s = np.zeros(n_sc_bins, dtype=float)
    hist_sc_p = np.zeros(n_sc_bins, dtype=float)
    ...
    if len(thetas_deg) > 0:
        hist_az_s /= len(thetas_deg)
        hist_az_p /= len(thetas_deg)
        hist_sc_s /= len(thetas_deg)
        hist_sc_p /= len(thetas_deg)

    hist_az = hist_az_s + hist_az_p
    hist_sc = hist_sc_s + hist_sc_p

    phase_az = hist_az / hist_az.sum() if hist_az.sum() > 0 else hist_az
    phase_sc = hist_sc / hist_sc.sum() if hist_sc.sum() > 0 else hist_sc

    denom_az = hist_az_s + hist_az_p
    denom_sc = hist_sc_s + hist_sc_p

    dolp_az = np.zeros_like(denom_az)
    dolp_sc = np.zeros_like(denom_sc)

    mask_az = denom_az > 0.0
    mask_sc = denom_sc > 0.0

    dolp_az[mask_az] = (hist_az_s[mask_az] - hist_az_p[mask_az]) / denom_az[mask_az]
    dolp_sc[mask_sc] = (hist_sc_s[mask_sc] - hist_sc_p[mask_sc]) / denom_sc[mask_sc]
    ...
    return {
        ...
        "dolp_azimuth": dolp_az,
        "scatter_centers_deg": sc_centers,
        "phase_scatter": phase_sc,
        "dolp_scatter": dolp_sc,
        "raw_azimuth": hist_az,
        "raw_azimuth_s": hist_az_s,
        "raw_azimuth_p": hist_az_p,
        "raw_scatter": hist_sc,
        "raw_scatter_s": hist_sc_s,
        "raw_scatter_p": hist_sc_p,
    }
...
def plot_results(result):
    ...
    plt.figure(figsize=(10, 4))
    plt.plot(sc_deg, dolp_sc)
    plt.xlabel("Scattering angle (deg)")
    plt.ylabel("DoLP")
    plt.title("Degree of linear polarization (scattering angle)")
    plt.xlim(0, 180)
    plt.ylim(-1.0, 1.0)
    plt.grid(True)
    plt.tight_layout()
