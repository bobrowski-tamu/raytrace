# -*- coding: utf-8 -*-
"""
Created on Mon Mar 30 12:33:20 2026

@author: jeff9
"""
# -*- coding: utf-8 -*-
"""
2D Ray Tracing for Hexagonal Ice Crystal
Corrected version - no sin(theta) weighting for 2D case
"""
import numpy as np
...
# =============================================================================
# FIND WHICH FACES RAY CAN HIT
# =============================================================================
def get_hittable_faces(ray_dir, normals):
    """
    A ray can hit a face if dot(ray_dir, normal) < 0
    (ray pointing toward the face)
    """
    hittable = []
    for i in range(6):
        if np.dot(ray_dir, normals[i]) < 0:
            hittable.append(i)
    return hittable
...
def enter_crystal(origin, direction, vertices, normals, n_out=1.0, n_in=1.31):
    ...
    # Find which faces the ray can hit (dot product < 0)
    hittable = get_hittable_faces(direction, normals)
...
def simulate():
    ...
    Rotate hexagon from 0 to 30 degrees (pi/6) due to 6-fold symmetry.
    """
    ...
    # Rotation angles (0 to 30 degrees due to symmetry)
    alphas = np.linspace(0, np.pi / 6, num_rot, endpoint=False)
    d_alpha = alphas[1] - alphas[0] if num_rot > 1 else np.pi / 6
    ...
    for alpha in alphas:
        ...
        for y in y_positions:
            ...
            for d, Is, Ip in exit_rays:
                ...
                # For incident ray along +x, angle is arccos(d_x)
                # Handle both forward and backward scattering
                d_norm = d / np.linalg.norm(d)
                cos_angle = np.clip(d_norm[0], -1, 1)
                
                # Determine angle based on y-component for sign
                angle = np.arccos(cos_angle)
                if d_norm[1] < 0:
                    angle = angle  # Keep as is (0 to pi covers both)
                
                # Find bin index
                idx = np.searchsorted(theta_bins, angle) - 1
                idx = np.clip(idx, 0, num_angles - 1)
                
                # Weight by rotation and impact parameter
                # In 2D, no sin(theta) weighting needed!
                weight = 1.0
                I_s_total[idx] += weight * Is
                I_p_total[idx] += weight * Ip
    
    # Normalize: (1/2π) ∫ P(φ) dφ = 1
    # Since we only integrate 0 to π and multiply by 2 for symmetry
...
# =============================================================================
# DIFFRACTION CORRECTION (YOUR FORMULA)
# =============================================================================
def diffraction_pattern(theta, chi=500):
    """
    Diffraction approximation for 2D hexagonal crystal.
    Integrates over rotation angles (0 to pi/6) due to hexagonal symmetry.
    """
    ...
    # Normalize: (1/2π) ∫ P(φ) dφ = 1 over full range
...
# Adjust weights as needed (0.5/0.5 or other ratios)
...
ax.plot(theta_deg, P_total, 'b-', alpha=0.3, label='Raw')
ax.plot(theta_deg, P_smooth, 'r-', linewidth=1.5, label='Smoothed')
ax.set_yscale('log')
ax.set_ylim(1e-4, 1e2)
ax.set_xlim(0, 180)
ax.set_xlabel('Scattering Angle (°)', fontsize=12)
ax.set_ylabel('Phase Function P(θ)', fontsize=12)
...
ax.set_title('Polarization (-P12/P11)', fontsize=14)
